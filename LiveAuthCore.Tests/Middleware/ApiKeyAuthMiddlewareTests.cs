...
public class ApiKeyAuthMiddlewareTests
{
    ...
    public async Task InvokeAsync_BypassPath_CallsNextWithoutApiKey(string path)
    {
        ...
        await middleware.InvokeAsync(context, services.ApiKeys);
    ...
    [Fact]
    public async Task InvokeAsync_MissingPublicKey_ReturnsUnauthorizedWithNoStore()
    {
        var nextCalled = false;
        var middleware = new ApiKeyAuthMiddleware(_ =>
        {
    ...
    [Fact]
    public async Task InvokeAsync_BlankPublicKey_ReturnsUnauthorized()
    {
    ...
    [Fact]
    public async Task InvokeAsync_InvalidPublicKey_ReturnsUnauthorized()
    {
        var middleware = new ApiKeyAuthMiddleware(_ => Task.CompletedTask);
        var services = CreateServices();
        var context = CreateContext("/api/private/resource", services);
        context.Request.Headers["X-LW-Public"] = "la_pk_missing";
    ...
    public async Task InvokeAsync_ValidProjectPublicKey_BindsProjectAndCallsNext()
    {
        ...
        var middleware = new ApiKeyAuthMiddleware(context =>
        {
            nextCalled = true;
    ...
    private static LiveAuthDbContext CreateDbContext()
    {
        var options = new DbContextOptionsBuilder<LiveAuthDbContext>()
            .UseInMemoryDatabase($"ApiKeyMiddlewareTests_{Guid.NewGuid():N}")
            .Options;

        return new LiveAuthDbContext(options);
    }

    private static Project CreateProject(string publicKey)
    {
        return new Project
        {
            Id = Guid.NewGuid(),
